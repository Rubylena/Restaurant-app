...
const Footer = () => {
  return (
    <div className='f_container'>
      ...
      <div class="f_row f_row-2">
          <h2>Social</h2>
          <div className='s_container'>
            {socialdata.map((Socials) => (
              <div className='s_social_container'>
                <img className='s_image' src={Socials.imageUrl} alt={Socials.name}/>
                <h3 className='s_text'>{Socials.name}</h3>
              </div>
            ))}
          </div>
      </div>
      <div class="f_row f_row-3">
          <h2>Stay up to date on the latest from Restaurant</h2>
          <input className={'email'} type={'email'} placeholder={'Enter your e-mail address'}/>
